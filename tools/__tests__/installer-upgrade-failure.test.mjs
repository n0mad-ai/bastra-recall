 ...
 *
 * The scripts are driven here with a stub `brew` (upgrade always fails) and a
 * healthy stub `bastra` (install/doctor/status all green), in an isolated HOME,
 * so the assertions are about the scripts alone: distinct incomplete wording,
 * non-zero exit, and — the part that matters for a machine that is still running
 * fine — no re-registration against a version that never landed.
 *
...
/** A stub `brew` whose `upgrade` fails (or succeeds, when UPGRADE_OK=1). */
...
/** A stub `bastra` that is perfectly healthy — on the OLD version. */
const BASTRA_STUB = `#!/usr/bin/env bash
echo "bastra $*" >> "$STUB_LOG"
case "$1" in
  --version) echo "0.9.2"; exit 0 ;;
...
async function runInstaller(script, { upgradeOk = false } = {}) {
...
for (const script of Object.keys(SCRIPTS)) {
  ...
  test(`#535 ${script}: a successful upgrade still finishes green`, async () => {
    const { code, out } = await runInstaller(script, { upgradeOk: true });
