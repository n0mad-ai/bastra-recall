...
export function detectRunner(root) {
  ...
  if (/node\s+(--\S+\s+)*--test\b/.test(script)) {
...
export function testFilesOf(dir, runner) {
  const globPart = runner.script.match(/\S*\*\S*/)?.[0] ?? "";
  const base = globPart.includes("/") ? globPart.slice(0, globPart.indexOf("*")).replace(/\/$/, "") : "";
  const roots = base !== "" && existsSync(join(dir, base)) ? [base] : ["test", "tests", "__tests__", "src"];
  const found = [];
...
export function resolveSpecifier(dir, fromFile, spec, packageNames = new Map()) {
  ...
  const probes = [base, ...SOURCE_EXTS.map((e) => base + e), ...SOURCE_EXTS.map((e) => join(base, `index${e}`))];
