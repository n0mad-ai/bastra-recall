...
- **Local tools** (CLI, MCP-forwarder — no `Origin` header) reach `/api/v1/*` without a token only when **both** are loopback: the peer socket *and* the `Host` header (`127.0.0.1` / `localhost` / `[::1]`, or no `Host` at all). A foreign `Host` over a loopback socket — DNS rebinding, or a local tunnel/reverse proxy — always needs the token (#526). Set `BASTRA_AUTH_LOOPBACK_SKIP=0` to require the token even for direct local callers.
...
- **DNS rebinding** is blocked on both surfaces: the token-less loopback endpoints (`/health`, `/hook/*`, `/vault/count`) answer only requests whose `Host` header is loopback, and `/api/v1/*` drops its token exemption for any non-loopback `Host` — a rebound page or a tunnel gets a `401`, not data. `BASTRA_ALLOWED_HOSTS` (comma-separated) opens the loopback-only endpoints for tunnel setups; it does **not** make `/api/v1/*` token-free for those hosts.
...
- **Lokale Tools** (CLI, MCP-Forwarder — kein `Origin`-Header) erreichen `/api/v1/*` nur dann ohne Token, wenn **beides** loopback ist: der Peer-Socket *und* der `Host`-Header (`127.0.0.1` / `localhost` / `[::1]`, oder gar kein `Host`). Ein fremder `Host` über einen Loopback-Socket — DNS-Rebinding oder ein lokaler Tunnel/Reverse-Proxy — braucht immer das Token (#526). Mit `BASTRA_AUTH_LOOPBACK_SKIP=0` wird das Token auch von direkten lokalen Aufrufern verlangt.
